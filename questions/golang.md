...
1.内存占用小：一个`Goroutine`的初始栈大小为`2kb`，而线程则需要1MB或更大内存，并且可以在运行过程中实现自动扩容，支撑了Go程序可以创建成千上万的`Goroutine`。
...
Go语言中强调要通过通信来共享内存而不是通过共享内存来通信，`channel`是Go中并发通信的媒介，使用`channel`通信可以避免共享内存带来的竞态和死锁问题，并且通过这种优雅实现使代码更加可读和易于维护。
